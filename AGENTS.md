...
- npm packages (Claude Code, openspec) are not in the image - the frigate-srv3 deploy script (vm-configs) npm-installs them as `abc` into `/home/abc/.local` (`.npmrc` prefix), so Claude Code can self-update. The image only provides Node.js/npm.
