...
The base image ships neither Node.js nor Claude Code. Using the distro's default Node package (rather than a pinned NodeSource LTS release) avoids trusting an extra external apt repository; simplicity was preferred over deliberate version pinning here. Claude Code itself is not version-pinned - unlike ESPHome, letting it self-update is acceptable.
