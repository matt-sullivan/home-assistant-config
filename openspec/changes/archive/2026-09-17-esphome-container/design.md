...
**Superseded: no longer pinned at all - tracks `:latest`.** Originally pinned by digest for full reproducibility (matching a "bump deliberately" philosophy). Reconsidered: rebuilds are already a deliberate, manually-triggered action (`docker/build-and-push.sh`), never automatic/scheduled, so a source-level pin adds a second manual step (bumping the pin) without adding any actual control over *when* a new version reaches production - that control already comes from choosing when to rebuild and push. Applies to every dependency in this image, not just the base image - see the "Latest stable dependency versions" requirement in `openspec/specs/esphome-dashboard-hosting/spec.md`.
...
**Superseded: switched back to `lscr.io/linuxserver/baseimage-ubuntu:resolute` + self-installed ESPHome - Alpine doesn't work for the whole LibreTiny/BK72xx (Beken) device family.**
Real device, not hypothetical: compiling any `bk72xx:` config failed with `Error relocating .../libgmp.so.10: obstack_vprintf: symbol not found` - the bundled ARM toolchain (`toolchain-gccarmnoneeabi`, glibc-linked) needs a symbol glibc bundles directly in `libc.so.6`, which Alpine's `gcompat` shim doesn't implement. Reproduced directly, then tried every suggested fix before concluding it was a dead end: `gcompat` was already present (base image ships it); added the community-recommended `libc6-compat`/`libstdc++` too, no change; tried Alpine's separate `musl-obstack` package specifically for the missing symbol - it exports `obstack_printf` but not the `_vprintf` variant this toolchain actually calls. No Alpine package provides it. ESP32 devices (Xtensa toolchain) were never affected - only this device family's ARM toolchain build happens to need this specific glibc-internal symbol.
`lscr.io/linuxserver/baseimage-ubuntu:resolute` (Ubuntu 26.04 LTS, current at time of writing - LSIO's codename-tagged builds are continuously rebuilt, no separate pin needed) is glibc-native, so this doesn't come up - confirmed by actually compiling the same `bk72xx:` config successfully on it. This also makes moot the earlier "someone else maintains the ESPHome version bump" reasoning for preferring Alpine: ESPHome is unpinned now regardless of base image (see the pinning Decision above), so self-installing it via `pip install esphome esphome-device-builder` (no version pin, `docker/Dockerfile`) costs nothing extra either way.
Two Alpine-specific fixes from the previous Decision no longer apply and were dropped, not carried forward: `usermod -p '*' abc` (Alpine's `abc` account starts password-locked; Ubuntu's doesn't - confirmed by testing SSH login without the fix) and commenting out a conflicting default `AuthorizedKeysFile` (Ubuntu's stock `sshd_config` doesn't set one). Node.js now comes from NodeSource's `setup_lts.x` (a rolling "current LTS" script, matching the no-pinning policy) rather than Alpine's `apk` index.
...
**Superseded: ESPHome tracks latest stable, unpinned - see the pinning Decision above.** Originally pinned to avoid surprise breakage from upstream releases. Reconsidered along with every other dependency in this image: rebuilds are already a deliberate, manually-triggered action, so choosing when to rebuild already provides the same control a pin would - a pin just adds a second manual bump step on top.
...
**Claude Code installed via NodeSource's `setup_lts.x` for Node.js, then `npm install -g @anthropic-ai/claude-code` - unpinned, tracking current LTS.**
The base image ships neither Node.js nor Claude Code, and Ubuntu's own default `nodejs` apt package freezes at a version well below Claude Code's declared minimum (>=22.0.0), confirmed via an `EBADENGINE` warning on install when first tried. NodeSource's setup script is the standard way to get a current Node on Debian/Ubuntu when the distro's own package is too old.
History: originally a hardcoded `setup_22.x` (superseded - no pinning anywhere in this image now, see the pinning Decision above); briefly moot during the Alpine base-image period, where `apk`'s own index already had current Node/npm (see the base-image Decision's "Superseded" note); back to NodeSource now that the base image is Ubuntu again, using `setup_lts.x` (NodeSource's own "whatever the current LTS is" script) instead of a hardcoded major version, consistent with tracking latest. Claude Code itself remains unpinned, letting it self-update.
