...
Chosen over a bare `ubuntu:24.04` build (would require re-implementing PUID/PGID, TZ/UMASK handling) and over forking imagegenius's full repo/CI (unnecessary overhead for a single personal image). imagegenius's image is a straight repackaging of the upstream `esphome` pip package, so ESPHome functionality is identical either way; only the base OS/user-mapping layer differs. The `ubuntu` variant was chosen partly because the existing ESPHome-CLI sshd setup runs in Alpine and has proven hard to configure; checked later and imagegenius's registry doesn't actually publish an alpine tag for this image (only `ubuntu`), so there was no real alternative to weigh against - Ubuntu 24.04 is the only base this image ships. Ubuntu itself isn't stale (current LTS, supported to 2029); switching to a genuinely different base (e.g. `debian:bookworm-slim`, the standard pick for "not Alpine, not bloated") would mean dropping imagegenius entirely and re-implementing PUID/PGID, TZ/UMASK, and the sshd/dashboard supervision it currently provides for free - not attempted here.
