...
**Base image: `lscr.io/linuxserver/baseimage-ubuntu:noble`, with ESPHome and `esphome-device-builder` installed via pip ourselves.**
Originally built `FROM ghcr.io/imagegenius/esphome:ubuntu-<version>-igN` (imagegenius's pre-built ESPHome image) rather than a bare `ubuntu:24.04` build, to get PUID/PGID and TZ/UMASK handling for free. That worked, but imagegenius stopped publishing Ubuntu-variant builds after ESPHome 2026.4.5 (`ig147`, ~May 2026) - their `ubuntu` tag never moved again, while upstream ESPHome continued on to 2026.9.0+ under their (now Alpine-based) bare version tags. Confirmed via the registry directly: `ghcr.io/imagegenius/esphome:2026.9.0`'s `/etc/os-release` is Alpine 3.24.1, and the `ubuntu-2026.4.5-ig147` tag's own image labels point at `https://docs.linuxserver.io/images/docker-baseimage-ubuntu` - imagegenius's esphome image was itself always just `FROM` that LinuxServer.io baseimage, with ESPHome layered on top.
So instead of staying frozen on an abandoned Ubuntu build, or switching to their actively-maintained Alpine image (reopening the exact musl/Alpine sshd-configuration pain that motivated picking Ubuntu originally), we go straight to `lscr.io/linuxserver/baseimage-ubuntu:noble` - the same base imagegenius used, but maintained directly by LinuxServer.io (rebuilt within the last week, vs. imagegenius's abandoned variant) - and install `esphome`/`esphome-device-builder` ourselves via `pip install --break-system-packages`. This decouples "which ESPHome version we run" from any third party's packaging cadence entirely; all PUID/PGID, TZ/UMASK, and s6-overlay supervision still come from the baseimage for free, same as before. Rootful-vs-rootless and other multi-service-container init systems (runit/phusion, supervisord) were considered and rejected: none of our rootless-Podman friction (SELinux, `userns=keep-id`) is base-image-dependent, and s6-overlay is already proven working via our own sshd service.
Both versions are pinned as Dockerfile `ARG`s (`ESPHOME_VERSION`, `DEVICE_BUILDER_VERSION`) and bumped manually, same philosophy as before.
